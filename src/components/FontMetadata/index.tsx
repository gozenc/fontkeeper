...
import { camelToSentence, debuggy, isNumeric } from "../../library/utils";
import React from "react";

export default function FontMetadata(props: any) {
    debuggy("FONT_INFO", props);
            ...
            <div className={styles.info}>
                {
                    data.map((item, i) => {
                        return (
                            <>
                                <span key={i} className={styles.key}>{item.key}</span>
...
function LinkOrText(props: any) {
    if (props.value.startsWith("http")) {
        return (
            <a target="_blank" className={styles.value} href={props.value}>{props.value}</a>
        );
    } else {
        return (
