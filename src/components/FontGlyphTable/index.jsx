...
export default function FontGlyphTable(props) {
  ...
  const { fonts, hasDefaults } = useFontsContext();

  const isDefaultFont = props.data === undefined;

  const [defaultChars, setDefaultChars] = React.useState(null);
  const [glyphCount, setGlyphCount] = React.useState(
    props.data
      ? props.data.nGlyphs
        ? props.data.nGlyphs
        : props.data.numGlyphs
        ? props.data.numGlyphs
        : "Glyph num not found"
      : 0
  );
  ...
  const defaultFamilyName = hasDefaults
    ? fonts[props.id - 1]?.name
    : props.title;
  ...
  return (
    <section>
      ...
      <div
        ...
        style={{
          fontFamily: props.title ? `${props.title.trim()}` : defaultFamilyName,
        }}
      >
        {defaultChars &&
        ...
        {!isDefaultFont &&
          !loading &&
          glyphs.map((font, i) => <FontGlyph key={i} data={font} />)}
      </div>
    </section>
  );
}
