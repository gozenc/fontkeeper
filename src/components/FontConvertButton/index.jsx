...
export default function FontConvertButton(props) {
    ...
    React.useEffect(() => {
        ...
        if (converted) {
            ...
            const freshResults = Object
                .keys(converted)
                .filter(key => key !== "url" && key !== "output" && key !== "fileName");

            setConvertedMessage(freshResults.reduce((acc, cur, i) => {
                return ` ${acc} ${camelToSentence(cur)}: ${values[i]} ${i === freshResults.length - 1 ? "" : "•"} `;
            }, "✅ "));
    ...
    const convertOpts = props.font.ext === "woff2"
        ? ["OTF", "WOFF"] : props.font.ext === "woff"
            ? ["OTF", "WOFF2"] : props.font.ext === "ttf"
                ? ["OTF", "WOFF", "WOFF2"] : ["WOFF", "WOFF2"];
                ...
                converted ? (
                    <>
                        <a href={converted.url}
                            ...
                            download={converted.fileName}>Download {converted.fileName}</a>
                    </>
                ) : (
                    <div className={classes(css.dFlex, styles.wrapper)}>
                        ...
                        <Icon onClick={handleConvert} type="button" name="play_circle_outline" />
                    </div>
                )
            }
        </div>
